    ...
    <el-table :data="list" v-loading="loading" border stripe>
      <el-table-column label="图片" width="180" align="center">
        <template #default="{ row }">
          <el-image :src="row.imgUrl" fit="cover" style="width: 120px; height: 60px; border-radius: 4px"
            :preview-src-list="[row.imgUrl]" preview-teleported />
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑轮播图' : '新增轮播图'" width="500px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="图片" prop="imgUrl">
          <el-upload class="banner-uploader" action="#" :show-file-list="false" :http-request="handleUpload"
            accept="image/*">
